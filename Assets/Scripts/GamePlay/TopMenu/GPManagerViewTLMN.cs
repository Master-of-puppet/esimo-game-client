...
public class GPManagerViewTLMN : EGPViewManager
{
    public override void OnStart()
    {
        OnSwapSlot(GameModelTLMN.ListPlayer.ToArray());
        DrawListWaitingPlayer();
        txtRoomName.value = ((LobbyTLMN)GameManager.Instance.selectedLobby).nameLobby;
        txtChangePassword.value = ((LobbyTLMN)GameManager.Instance.selectedLobby).config.password;
    }
    public override void OnUpdate()
    {
        if ((string.IsNullOrEmpty(txtRoomName.value) || txtRoomName.value == ((LobbyTLMN)GameManager.Instance.selectedLobby).nameLobby) && string.IsNullOrEmpty(txtChangePassword.value))
        {
    ...
    public override void OnClickSave(GameObject go)
    {
        ...
        if (((string.IsNullOrEmpty(((LobbyTLMN)GameManager.Instance.selectedLobby).config.password)) && (string.IsNullOrEmpty(txtChangePassword.value) || txtChangePassword.value == ((LobbyTLMN)GameManager.Instance.selectedLobby).config.password))
    ...
    public override void SetDataPlayer(EPlayerController p, int i)
    {
        ...
        int slot = GameModelTLMN.YourController.slotServer;
        slot += (i + 1);
        if (slot >= 4)
            slot -= 4;
    ...
    IEnumerator _DrawListWaitingPlayer()
    {
        ...
        int i = 0;
        //foreach (PlayerControllerChan player in GameModelTLMN.ListWaitingPlayer)
        //{
